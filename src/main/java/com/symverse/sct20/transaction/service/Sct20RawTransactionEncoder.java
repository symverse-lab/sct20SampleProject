...
public class Sct20RawTransactionEncoder {
	
	static final Logger logger = LoggerFactory.getLogger(Sct20RawTransactionEncoder.class);
    ...
    public static byte[] signMessage(Sct20SendRawTransaction sct20SendRawTransaction, byte chainId, Credentials credentials ) {
    ...
    public static byte[] hashEncode(Sct20SendRawTransaction sct20SendRawTransaction, byte chainId  ) {
    ...
    private static byte[] hashEncode(Sct20SendRawTransaction sct20SendRawTransaction, Sign.SignatureData signatureData) {
		List<RlpType> values = hashAsRlpValues(sct20SendRawTransaction, signatureData );        
		RlpList rlpList = new RlpList(values);
		return RlpEncoder.encode(rlpList);
    }
    
    public static byte[] encode(Sct20SendRawTransaction sct20SendRawTransaction, byte chainId) {
        Sign.SignatureData signatureData = new Sign.SignatureData(chainId, new byte[] {}, new byte[] {});
        return encode(sct20SendRawTransaction, signatureData);
    }

    private static byte[] encode(Sct20SendRawTransaction sct20SendRawTransaction, Sign.SignatureData signatureData ) {
		List<RlpType> values = hashAsRlpValues(sct20SendRawTransaction, signatureData);
		RlpList rlpList = new RlpList(values);
		return RlpEncoder.encode(rlpList);
    }
    ...
    /**
     *  input 파라미터 생성
     * @param Sct20SendRawTransaction methodType1 토큰 교환 - 송금 
     * @return
     */
    static byte[] hashInputRlpMethodSendSym_Values(Sct20SendRawTransaction sct20SendRawTransaction) {  
    ...
    /**
     * @param Sct20SendRawTransaction methodType 0 ( SCT 20 생성 - 토큰 생성 ) 
     * @return 
     */
    static byte[] hashInputRlpMethodType0_Values(Sct20SendRawTransaction sct20SendRawTransaction) {
    ...
    /**
     * @param Sct20SendRawTransaction methodType 1 , 3 , 4, 5  ( 1 - 토큰 송금 , 3 - 제 3자 토큰 승인 ( 스펜더 지정 ) , 4 - 토큰 추가 발행 , 5 - 토큰 태움 ) 
     * @return 
     */
    static byte[] hashInputRlpMethodType1and3and4and5_Values(Sct20SendRawTransaction sct20SendRawTransaction) {  
    	   List<RlpType> setValue = new ArrayList<>();
    	   setValue.add( RlpString.create( Numeric.hexStringToByteArray( CommUtil.strintDecimaltoHexValueConvert ( sct20SendRawTransaction.getInput().getSctType() )))  );
           setValue.add( RlpString.create( Numeric.toBigInt( sct20SendRawTransaction.getInput().getMethod() ) ));
    	   List<RlpType> contents = new ArrayList<RlpType>();
    ...
    /**
     * @param Sct20SendRawTransaction methodType 2  ( 제 3자 토큰 송금 - 제 3자 토큰 승인한 ( 스펜더 ) 계정에서 토큰 송금 ) 
     * @return 
     */
    static byte[] hashInputRlpMethodType2_Values(Sct20SendRawTransaction sct20SendRawTransaction) {  
    	   List<RlpType> setValue = new ArrayList<>();
    	   setValue.add( RlpString.create( Numeric.hexStringToByteArray( CommUtil.strintDecimaltoHexValueConvert ( sct20SendRawTransaction.getInput().getSctType() )))  );
           setValue.add( RlpString.create( Numeric.toBigInt( sct20SendRawTransaction.getInput().getMethod() ) ));
    	   List<RlpType> contents = new ArrayList<RlpType>();
     ...
     * Sct20SendRawTransaction methodType 6 , 7  ( 6 - 토큰 거래 일시 정지 , 7 - 토큰 거래 일시 정지 해제 ) 
     * @return
     */
    static byte[] hashInputRlpMethodType6and7_Values(Sct20SendRawTransaction sct20SendRawTransaction) {  
    	   List<RlpType> setValue = new ArrayList<>();
    	   setValue.add( RlpString.create( Numeric.hexStringToByteArray( CommUtil.strintDecimaltoHexValueConvert ( sct20SendRawTransaction.getInput().getSctType() )))  );
           setValue.add( RlpString.create( Numeric.toBigInt( sct20SendRawTransaction.getInput().getMethod() ) ));
    	   List<RlpType> contents = new ArrayList<RlpType>();
     ...
     * @return List<RlpType>
     */
    static List<RlpType> hashWorkNodeRlpValues(Sct20SendRawTransaction sct20SendRawTransaction) {
     ...
     * @return List<RlpType>
     */
    static List<RlpType> hashWorkNodeRlpValuesTest(Sct20SendRawTransaction sct20SendRawTransaction) {
       List<RlpType> result = new ArrayList<>();
    ...
    static List<RlpType> hashAsRlpValues(Sct20SendRawTransaction sct20SendRawTransaction, Sign.SignatureData signatureData ) {
    	List<RlpType> result = new ArrayList<>();
        ...
        if( methodType.equals("0") ) {
           ...
           result.add(RlpString.create( hashInputRlpMethodType6and7_Values(sct20SendRawTransaction) ) ); // input
            ...
            result.add(RlpString.create( hashInputRlpMethodSendSym_Values(sct20SendRawTransaction) ) ); // input
        }
